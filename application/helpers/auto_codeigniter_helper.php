<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	if(!function_exists('is_mobile'))
	{
		function is_mobile($mobile=''){
			...
			if (strlen ( $mobile ) != 11 || ! preg_match ( '/^1[3|4|5|8][0-9]\d{4,8}$/', $mobile )) {  
				return false;  
			} else {  
				return true;  
			}  
	...
	{
		function show_photo($url,$default_img = 'nopic.gif'){
		
			if(str_exists($url,UPLOAD_URL))
			{
				$imgurl_replace= str_replace(UPLOAD_URL, '', $url);	
				
				if(!is_file(UPLOAD_PATH.$imgurl_replace))return IMG_PATH.$default_img;
				if(!file_exists(UPLOAD_PATH.$imgurl_replace)) return IMG_PATH.$default_img;
	...
	if(!function_exists('show_avatar'))
	{
		...
		{
			return  show_photo(UPLOAD_URL.'user/'.$img,'avatar.png');
		}
	}
