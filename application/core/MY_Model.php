...
class Base_Model extends MY_Model{
	 ...
	 *  缓存SQL进文件
	 */
	final public function set_cache_select_in_file($cache_name='',$where = '', $data = '*', $limit = '', $order = '', $group = '', $key='')
	{
		$data_list = $this->Seller_model->select($where,$data,$limit,$order,$group,$key);
		setcache($cache_name,$data_list);
	...
	final public function get_one_field_value($where = '', $field_name , $order = '', $group = '') {
		$datainfo= $this->select($where , $field_name, '1', $order , $group);
		if(count($datainfo)>0)$datainfo=$datainfo[0];
		
		return isset($datainfo[$field_name])?$datainfo[$field_name]:NULL;
	}
