...
;(function($, undefined) {
	...
	$[pluginName] = function(message, type) {
		...
		if (type === undefined || type == $[pluginName].TYPE_ERROR) {
			...
			$[pluginName].options.delay = 4000;
		} else if (type == $[pluginName].TYPE_OK) {
			$selector.removeClass($[pluginName].options.errClass).addClass($[pluginName].options.okClass);
			$[pluginName].options.delay = 4000;
		}else if (type == $[pluginName].TYPE_WAIT) {
			$selector.removeClass($[pluginName].options.errClass).addClass($[pluginName].options.waitClass);
			$[pluginName].options.delay = 60000;
		}
