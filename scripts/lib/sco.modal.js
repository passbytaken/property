...
;(function($, undefined) {
	...
	$.extend(Modal.prototype, {
		...
		if (!this.$modal.length) {
			...
			this.$modal = $('<div class="modal fade bs-example-modal-sm" id="' + this.options.target.substr(1) + ' tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">'
							  +'<div class="modal-dialog">'
							  +'<div class="modal-content">'
							 +'  <div class="modal-header">'
							 +'	<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'
							 +'	<h4 class="modal-title" style="color:black" >New message</h4>'
							 +'  </div>'
							 +'  <div class="modal-body">'
							 +'  <div class="inner" style="color:black"></div>'	
							 +' </div>'
			...
			if (typeof(this.options.ok.callback) == "function") {
			   $("<button type='button' class='btn btn-primary' >"+this.options.ok.label+"</button>").prependTo(".modal-footer").click(this.options.ok.callback);
			}  
